...
//if ($_SERVER['REQUEST_METHOD'] == 'GET') {
  ...
  if ( isset($_REQUEST['todo']) ){
    ...
    switch($todo){
      case 'readmovie':
        if ($_SERVER['REQUEST_METHOD'] != 'GET') { bad_http_verb('GET'); exit(); }
        $categorie = '';
        $profile = '';
        $min_age = '';
        $favorite = '';
      ...
      case 'addMovie':
        if ($_SERVER['REQUEST_METHOD'] != 'POST') { bad_http_verb('POST'); exit(); }
      ...
      case 'updateProfile':
        if ($_SERVER['REQUEST_METHOD'] != 'PUT') { bad_http_verb('PUT'); exit(); }
        // HTTP verb = 'PUT'
      ...
      case 'deleteFavorite':
        if ($_SERVER['REQUEST_METHOD'] != 'DELETE') { bad_http_verb('DELETE'); exit(); }
        // HTTP verb = 'DELETE'
        $data = json_decode(file_get_contents('php://input'), true);
